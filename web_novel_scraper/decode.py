...
class Decoder:
    ...
    def get_chapter_content(
        self, html: str, save_title_to_content: bool, chapter_title: str
    ) -> str:
        """
    ...
    @staticmethod
    def host_exists_in_decode_guide(host: str, decode_guide_file: str) -> None:
        load_decode_guide(path=decode_guide_file, host=host)
    ...
    def _load_decode_guide(self) -> None:
        ...
        #     logger.error(
        #         f"Decode Guide for Host {self.host} not found on Decode Guide File {self.decode_guide_file}"
        #     )
        #     raise DecodeGuideError(
        #         f"Error getting Decode Guide for Host {self.host} from Decode Guide File {self.decode_guide_file}: {str(e)}"
        #     )

        # except DecodeGuideIsEmptyError as e:
        #     logger.error(f"Decode Guide File is empty {self.decode_guide_file}")
        #     raise DecodeGuideError(
        #         f"Error getting Decode Guide for Host {self.host} from Decode Guide File {self.decode_guide_file}: {str(e)}"
        #     )

        # except DecodeGuideNotFoundError as e:
        #     logger.error(f"Decode Guide File not found at {self.decode_guide_file}")
        #     raise DecodeGuideError(
        #         f"Error getting Decode Guide for Host {self.host} from Decode Guide File {self.decode_guide_file}: {str(e)}"
        #     )

        # except LoadDecodeGuideError as e:
        #     logger.error(f"Error loading Decode Guide file {self.decode_guide_file}")
        #     raise DecodeGuideError(
        #         f"Error getting Decode Guide for Host {self.host} from Decode Guide File {self.decode_guide_file}: {str(e)}"
        #     )
            ...
            if extract["type"] == "attr":
                ...
                for element in elements_aux:
                    try:
                        ...
                        pass
                logger.debug(
                    f'{len(elements)} elements found using attribute "{attr_key}"'
                )
