...
class Novel:
    ...
    def load(
        ...
        try:
            novel.decoder = Decoder(novel.host, novel.cfg.decode_guide_file)
    ...
    def new(
        ...
        if toc_html and not host:
            raise ValidationError(
                "When providing toc_html, host must be explicitly provided"
            )
        ...
        try:
            Decoder.host_exists_in_decode_guide(
                host=novel.host, decode_guide_file=novel.config.decode_guide_file
            )
        except DecodeGuideError as e:
            logger.debug("Traceback:", exc_info=True)
            raise ScraperError(e) from e
    ...
    def set_toc_main_url(self, toc_main_url: str, update_host: bool = True) -> None:
        ...
        if self.toc_main_url == toc_main_url:
            logger.warning(
                f"Tried to update TOC Main URL to {toc_main_url}, but is the same as current value."
            )
            return

        self.toc_main_url = toc_main_url
        logger.info(
            f'TOC Main URL updated to "{self.toc_main_url}", TOCs already requested will be deleted.'
        )
        self.delete_toc()
    ...
    def add_toc_html(self, html: str, host: str = None) -> None:
        ...
        if self.toc_main_url:
            logger.debug(
                "TOC main URL is exclusive with manual TOC files, TOC main URL will be deleted."
            )
            self.delete_toc()
        ...
        if host:
            ...
            if self.host is None:
                logger.error(
                    "When using TOC files instead of URLs, host must be provided."
                )
                raise ValidationError(
                    "Host must be provided when using TOC files instead of URLs."
                )
        self.novel_data_helper.add_toc_fragment(html)
    ...
    def sync_toc(self, reload_files: bool = True) -> None:
        ...
        reload_files = (
            reload_files or all_tocs_content is None
        ) or self.toc_main_url is not None
        if reload_files:
    ...
    def scrap_chapter(self, chapter: Chapter, reload_file: bool = False) -> Chapter:
        ...
        save_title_to_content = (
            ...
            or self.decoder.save_title_to_content()
        )
        try:
            chapter = self._decode_chapter(
                chapter=chapter, save_title_to_content=save_title_to_content
            )
        except DecodeError as e:
            logger.error(
    ...
    def request_all_chapters(
        ...
        if sync_toc:
            ...
            try:
                ...
                logger.warning(
                    "Error when trying to sync TOC, continuing without syncing..."
                )
    ...
    def _decode_chapter(
        self, chapter: Chapter, save_title_to_content: bool = False
    ) -> Chapter:
        """
        ...
        try:
            chapter.chapter_content = self.decoder.get_chapter_content(
                chapter.chapter_html, save_title_to_content, chapter.chapter_title
            )
        except DecodeError:
            logger.error(
