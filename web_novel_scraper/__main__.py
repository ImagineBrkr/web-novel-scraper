...
def create_toc_html_option(required: bool = False):
    return click.option(
        ...
        required=required,
        help=(
            "Novel TOC HTML loaded from file."
            if required
            else "Novel TOC HTML loaded from file (required if not loading from URL)"
        ),
    )
...
save_title_to_content_option = click.option(
    "--save-title-to-content",
    is_flag=True,
    show_default=True,
    default=False,
    help="Add the chapter title to the content.",
)
...
def create_novel(
    ...
    if not toc_main_url and not toc_html:
        click.echo("You must provide either a TOC URL or a TOC HTML file.", err=True)
        return
...
def set_scraper_behavior(
    ctx, title, save_title_to_content, auto_add_host, force_flaresolver, hard_clean
):
...
def delete_toc(ctx, title, auto_approve):
    ...
    if not auto_approve:
        click.confirm(
            f"Are you sure you want to delete the TOC for {title}?", abort=True
        )
    novel.delete_toc()
    novel.save_novel()
...
def request_all_chapters(ctx, title, sync_toc, update_html, clean_chapters):
    ...
    novel.request_all_chapters(
        sync_toc=sync_toc, reload_files=update_html, clean_chapters=clean_chapters
    )
    novel.save_novel()
