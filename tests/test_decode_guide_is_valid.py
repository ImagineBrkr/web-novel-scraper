...
DECODE_GUIDE_PATH = (
    Path(__file__).parent.parent / "web_novel_scraper/decode_guide/decode_guide.json"
)
...
@pytest.mark.parametrize(
    "entry", decode_guide_entries, ids=lambda e: e.get("host", "<unknown>")
)
def test_decode_guide_entry_schema(entry):
    ...
    for bool_key in OPTIONAL_BOOLEAN_FLAGS:
        if bool_key in entry:
            assert isinstance(entry[bool_key], bool), (
                f"'{bool_key}' must be boolean if present"
            )
    ...
    if "request_config" in entry:
        ...
        if "force_flaresolver" in rc:
            assert isinstance(rc["force_flaresolver"], bool), (
                "'force_flaresolver' must be boolean"
            )
        if "request_timeout" in rc:
            assert (
                isinstance(rc["request_timeout"], (int, float))
                and rc["request_timeout"] > 0
            ), "'request_timeout' must be a number > 0"
        if "request_time_between_retries" in rc:
            assert (
                isinstance(rc["request_time_between_retries"], (int, float))
                and rc["request_time_between_retries"] >= 0
            ), "'request_time_between_retries' must be a number >= 0"
        if "request_retries" in rc:
            assert (
                isinstance(rc["request_retries"], int) and rc["request_retries"] > 1
    ...
    def validate_section(sec_cfg, name):
        ...
        if sec_cfg.get("use_custom_processor"):
            ...
            assert not other_keys, (
                f"When 'use_custom_processor' is true in {name}, no other keys are allowed"
            )
            return
        ...
        for key in ("element", "id", "class", "selector", "attributes"):
            if key in sec_cfg and sec_cfg[key] is not None:
                assert isinstance(sec_cfg[key], str), (
                    f"'{key}' in {name} must be a string or null"
                )
        if "array" in sec_cfg:
            assert isinstance(sec_cfg["array"], bool), (
                f"'array' in {name} must be boolean"
            )
        if "inverted" in sec_cfg:
            assert isinstance(sec_cfg["inverted"], bool), (
                f"'inverted' in {name} must be boolean"
            )
        # Validate extract
    ...
    for section in REQUIRED_SECTIONS:
        assert section in entry and isinstance(entry[section], dict), (
            f"Each entry must have a '{section}' dictionary"
        )
    ...
    for section in OPTIONAL_SECTIONS:
        if section in entry:
            assert isinstance(entry[section], dict), (
                f"{section} must be an dictionary if present"
            )
            validate_section(entry[section], section)
